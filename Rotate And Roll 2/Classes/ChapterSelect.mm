...
- (void)onSelectChapter:(CCMenuItemImage *)sender
{
    ...
    if(sender.tag)
    {
        ...
        NSLog(@"SELECTED PAGE: %d", sender.tag);
        
        World *tmpworld  = [worldsArray objectAtIndex:sender.tag-1];	// MUNDO ACTUAL
        
        if(tmpworld.locked == FALSE)
        {
...
- (id)init {
    ...
    if( (self=[super init]))
    {
        ...
        CCSprite* fondo2 = [CCSprite spriteWithFile:@"fondomundo1.png"];
