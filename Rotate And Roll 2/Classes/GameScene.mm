...
-(void)afterStep {
    ...
    for(pos = _contactListener->_contacts.begin(); pos != _contactListener->_contacts.end(); ++pos)
    {
        ...
        if (bodyA->GetUserData() != NULL && bodyB->GetUserData() != NULL)
        {
            ...
            if (spriteOther.tag != TAG_DESIGN
                && spriteOther.tag != TAG_KEY
                && spriteOther.tag != DEFAULT_TAG
                && spriteOther.tag != TAG_TELEPORT) {moving = false;}
...
-(void) tick: (ccTime) dt
{
    ...
    if ([following body]) {
        ...
        if (flightSpeed!=0) {
            ...
            [following transformRotation: -flo];
            following.scaleY = vc.x>0? .3f:-.3f;
            following.scaleX = .3f;
        }
        else {
        
            [following transformRotation: 0];
            following.scaleX = vc.x>0? .3f:-.3f;
            following.scaleY = .3f;
        }
...
- (void) checkBall {
    ...
    if (ball) {
        ...
        float actualSpeed = sqrtf(powf(velocity.x,2) + powf(velocity.y,2));
        
        
        if (moving) {
            cupido.position = ball.position;
            [self drawLines];
            [self correctBirdAngle:actualSpeed];
        }
        else {
            flightSpeed = 0;
            line1.visible = false;
            line2.visible = false;
            line3.visible = false;
            cupido.visible = false;
            movedCircle.visible = false;
        }
        

        
        BOOL gravityOn = (!moving)||velocity.y>0;
        ball.body -> SetGravityScale(1.25f);
        
        [self moveSelfPosition];
        [self checkLostGame];
...
- (void) correctBirdAngle: (float) birdForce {
    ...
    if (flightSpeed==0) {
        ...
        if (birdForce > flightSpeed && birdForce > 0) {
            
            flightSpeed = birdForce;
        }
        else {
            return;
        }
    ...
    CGPoint vector = ccpSub(originPoint, movingPosition);
    vector = ccp(vector.x/2, vector.y/2);
    float distanciaLinea = sqrtf(powf(vector.x,2) + powf(vector.y,2));
    
    if (distanciaLinea<15) {
        return;
    }

    float angle = ccpToAngle(vector);
    CGPoint angulate = ccpForAngle(angle);
    vector = ccp(angulate.x * flightSpeed, angulate.y * flightSpeed);
    ball.body -> SetLinearVelocity(b2Vec2(-vector.x, -vector.y));
}
...
- (void)ccTouchesBegan:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    for( UITouch *touch in touches ) {
        ...
        location = ccp(location.x + self.position.x, location.y);
        originPoint = location;
        movingPosition = location;
        moving = true;
    }
...
- (void)ccTouchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    moving = false;
        ...
        if (!ball) {
        ball = [lh createSpriteWithName:@"ball" fromSheet:@"UntitledSheet" fromSHFile:@"items.pshs"];
        [ball prepareAnimationNamed:@"bord" fromSHScene:@"flappy.pshs"];
        ball.scale = .3f;
        [ball playAnimation];
        [ball transformPosition: cupido.position];
        }
        ...
        if (numberOfShoots == 0) {  ball.color = ccWHITE;  }
        else {
            ball.color = ccGREEN;
        }
        ...
        [ball body]->SetAngularDamping( 3 );
        [ball body]->SetAngularVelocity(0);
        //[arrow body]->SetBullet(true);

        [ball setZOrder:20];
        [ball body] -> SetLinearVelocity(-2*shootImpulse);
        ball.tag = TAG_ARROW;
...
- (void)ccTouchesMoved:(NSSet *)touches withEvent:(UIEvent *)event{
    ...
    for( UITouch *touch in touches ) {
        ...
        location = ccp(location.x + self.position.x, location.y);
        if (ball) {
            originPoint = ccp(ball.position.x + self.position.x, ball.position.y);
        }
        movingPosition = location;
        [self drawLines];
...
- (void) drawLines {
    ...
    CGFloat rotateAngle = -ccpToAngle(vector)+3.14159;
