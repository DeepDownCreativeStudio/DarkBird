...
-(id) init
{
	if( (self=[super init]))
	{
        ...
        [SimpleAudioEngine sharedEngine].effectsVolume = .8f;
	...
	//	[[GameHUD sharedManager] setGame:self];
        ...
        bg2 = [lh createSpriteWithName:@"bgf" fromSheet:@"bg2" fromSHFile:@"bg.pshs"];
        ...
        bg = [lh createSpriteWithName:@"bgf" fromSheet:@"bg2" fromSHFile:@"bg.pshs"];
...
-(void) setupCollisionHandling {
    ...
    [lh registerBeginOrEndCollisionCallbackBetweenTagA:TAG_ARROW andTagB: TAG_LOST idListener:self selListener:
     @selector(arrowLost:)];
    [lh registerBeginOrEndCollisionCallbackBetweenTagA:TAG_LOST andTagB: TAG_ARROW idListener:self selListener:
     @selector(arrowLost:)];
...
-(void) StartLevel:(NSString*) level
{
    ...
    if (fondoapplied) { fondoapplied=false;
            ...
            if (spriteB.tag == TAG_ARROW)
            {
                ...
                if(spriteA.tag == TAG_LOST)
                {
                    ...
                    [spriteB removeSelf];
                        ...
                        [bobo runAction:[CCFadeTo actionWithDuration:1.5 opacity:160]];
                        resetbo = true;
                    [[SimpleAudioEngine sharedEngine] playEffect:@"offscreen.mp3"];
...
-(void) tick: (ccTime) dt
{
    ...
    if (blackout) {
        if (rand() % 100 < 30) {
        bg.opacity = 140 + (bg.opacity - 140)* 0.99f;
        }
    }
    ...
    if (repeat>0) {
        ...
        if (!noSeguir) {
            ball = [lh spriteWithUniqueName:@"arrow"];
        ...
        if (ball != nil) {
            ...
            if ([ball body] -> GetLinearVelocity().x < 0) {
                ...
                NSLog(@"- sense %f", [ball body] -> GetLinearVelocity().x );
                
                [SimpleAudioEngine sharedEngine].effectsVolume = 1.2f;
                
                if ([ball body] -> GetLinearVelocity().x < -4.5f) {
                   [[SimpleAudioEngine sharedEngine] playEffect:@"wound.mp3"];   
                } else {
                    [[SimpleAudioEngine sharedEngine] playEffect:@"broken.mp3"];
                }
                CCParticleSystem* particleSystem;
                particleSystem = [CCParticleSystemQuad particleWithFile:@"puck.plist"];
                particleSystem.positionType = kCCPositionTypeGrouped;
                particleSystem.position = ball.position;
                particleSystem.scale = .3f;
                [self addChild: particleSystem z:100];
...
-(void) first {
    if (!first) {
        ...
        [arrow prepareAnimationNamed:@"bord" fromSHScene:@"flappy.pshs"];
        [arrow setAnimationDelayPerUnit:.02];
        [arrow playAnimation];
        arrow.uniqueName = @"arrow";
        arrow.scale = .5;
        ball = arrow;
        arrow.color = ccWHITE;
        ...
        [arrow setZOrder:20];
        [arrow body] -> SetLinearVelocity(b2Vec2(5,5));
        arrow.tag = TAG_ARROW;
...
-(void) ganador {
    ...
    [[SimpleAudioEngine sharedEngine] playEffect:@"teleport.mp3"];
    timer = nil;
...
-(void) dispararFlecha
{
    ...
    CCSprite* bobo = [CCSprite spriteWithFile:@"flecha2.png"];;
    bobo.tag = 0;
    [self addChild:bobo z:-1];
    
    b2BodyDef bodyDef;
    bodyDef.type = b2_dynamicBody;
    bodyDef.userData = bobo;
    //bodyDef.position.Set( 0, 5 );
    b2Vec2 position = [cupido body]->GetPosition();
    bodyDef.position.Set(position.x,position.y);
    bodyDef.angle = [cupido body]->GetAngle();
    ...
    m_loadedArrowBody = world->CreateBody(&bodyDef);
    ...
    NSLog(@"BODIES COUNT: %lu", m_arrowBodies.size());
    
    tiros++;
	CCLOG(@"Disparando Flecha TIROS:%i", tiros);
    CCSprite *sprite = (CCSprite *) m_loadedArrowBody->GetUserData();
    sprite.tag = TAG_ARROW;
    shooting = TRUE;
    
    // m_loadedArrowBody->SetAwake(true);
    m_loadedArrowBody->SetGravityScale(1);
    m_loadedArrowBody->SetAngularVelocity(0);
    m_loadedArrowBody->SetTransform( [cupido body]->GetWorldPoint( b2Vec2(3,0) ), [cupido body]->GetAngle() );
    m_loadedArrowBody->SetLinearVelocity(shootImpulse);
...
- (void)ccTouchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
        if (resetbo) {
            ...
            NSLog(@"ENDED");
            for (LHSprite* s in [lh spritesWithTag:TAG_ARROW]) {
                b2Vec2 primary = [s body] -> GetLinearVelocity();
                primary = b2Vec2 (5, 10);
                [s body] -> SetLinearVelocity(primary);
                int a = rand() %70;
                if (a<50) {
...
-(void) gotoLevelMenu
{
    ...
    [[CCDirector sharedDirector] replaceScene:[CCTransitionFade transitionWithDuration:0.5 scene:[LevelScene node] ]];
}
